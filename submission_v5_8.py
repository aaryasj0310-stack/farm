"""
🌾 Kaggriculture Master Submission Agent (P-FINAL-v5.8 Release Candidate)
Authoritative Live-Calibrated Composite uniting the proven multi-channel Kaggle
execution engine with out-of-loop dynamic pricing, non-blocking market sniping,
legal-seed crop allocation, and pasture repair controllers.
...
# ==============================================================================
# 3. REPAIRED OUT-OF-LOOP DYNAMIC MARKET & SNIPING CONTROLLERS
# ==============================================================================

def _update_turn_price_history(obs: Dict[str, Any]) -> None:
    """Update moving average STRICTLY ONCE PER TURN."""
...
def _sort_market_v58(action: Dict[str, Any], obs: Dict[str, Any]) -> Dict[str, Any]:
    """Live-calibrated dynamic spot pricing & priority-sorter market sniping."""
    ...
    for index, order in enumerate(orders):
        if order and order[0] == "SELL" and len(order) >= 3:
            ...
            # Non-blocking priority sorting: prioritize premium sales without dropping orders
...
# ==============================================================================
# 4. REACTION, WEATHER HEDGING, & WEED REPAIR CONTROLLERS
# ==============================================================================
...
def _repair_crop_and_weather_v58(obs: Dict[str, Any], action: Dict[str, Any]) -> Dict[str, Any]:
...
def agent(obs: Dict[str, Any], config: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    action = _sort_market_v58(action, obs)
    action = _repair_pasture(obs, action)
    action = _repair_crop_and_weather_v58(obs, action)
    return action
