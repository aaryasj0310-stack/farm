...
**Rule P3 — Same-turn seeding (the anti-starvation invariant).** On the very turn SW is bought, immediately spend ESCROW: order 15 × WHEAT ($10) and plant the 10 tiles closest to (4,5) first (rows y=7, then y=6 pastures already placed, then y=8…). Seed orders are placed *before* hiring extra hands, so a Fibonacci hire spike can never abort them.
...
- `STOCK` = wheat in shed + wheat on SW/NW tiles maturing (harvestable) on or before D29.
...
def sw_plant_decision(D, free_tiles, STOCK, H):
    if D >= 28: return {"wheat": 0, "carrot": 0}                 # fallow
    feed_need = H * (29 - D + 1)
    n_wheat = 0
    if D <= 25 and STOCK < feed_need:
        n_wheat = min(free_tiles, -(-（feed_need - STOCK) // 5))  # ceil div
    n_carrot = free_tiles - n_wheat
    if D == 27 and (0.5 * 3.5 * 70 - 20) <= 0: n_carrot = 0      # EV gate
