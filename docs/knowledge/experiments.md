...
### SW-forward architecture Phase A-R correctness hardening (2026-09-23)
- Type: EXPERIMENT / HARDENING
- Source: docs/sw_shadow_validation.md; strategy/resource_ledger.py; strategy/cohort_planner.py; strategy/service_certificate.py; main.py; tests/test_sw_forward_architecture.py; tests/test_sw_historical_failure_scenarios.py
- Confidence: HIGH (1,109/1,109 full test suite passing; 30/30 SW architecture and failure replay tests passing; zero divergence across discovery panel)
- Details: Completed Phase A-R correctness hardening across 4 critical gaps: (1) Feed timing & wheat harvest causality: eliminated `getattr(t, "age", 0)` bug in favor of authoritative `crop_age(t, day)` and `t.planted_day`. Separated `projected_mature_wheat` from `projected_accessible_wheat` and modeled same-day physical chain (Worker -> Wheat -> HARVEST -> Animal -> FEED <= 23). (2) Physically causal storage: implemented `WorkerStorageState` tracking individual worker positions, shed distances, and carried goods; enforced rule that market SELL cannot pull from backpacks; modeled midnight auto-drop and discard mechanics when inventory exceeds 100 units. (3) Engine-exact economics in `CohortPlanner`: integrated `market/price_math.py` (`market_price`, `total_revenue_estimate`), eliminated double-counting of cannibalization in $\Delta FC = \text{WITH} - \text{WITHOUT}$, and preserved explicit counterfactual cohort values. (4) Strengthened `ServiceCertificate` with explicit task dependencies (`prerequisite_task_id`, `earliest_start_hour`) enforcing causal execution ordering. Ran 3-seed shadow sanity panel on discovery seeds (96401, 42, 100) for 72 steps: confirmed 0 divergences from OFF baseline, 72 shadow evaluations per game, and sound telemetry ($p50 < 0.35\text{ ms}$). Maintained 100% byte-for-byte parity between `agent/` and `submission/`. Protected seeds `98001–98050` strictly untouched.
- Related: docs/sw_shadow_validation.md; docs/sw_resource_ledger_spec.md; docs/sw_cohort_planner_spec.md; docs/sw_service_certificate_spec.md
