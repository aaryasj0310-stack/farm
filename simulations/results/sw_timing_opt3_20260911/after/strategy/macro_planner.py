...
class MacroPlanner:
    ...
    def build(self, ctx, boosts=None, opp_advice=None):
        ...
        # Keep building the already-funded SW pasture tranche through D13;
        # no new livestock is bought after the cutoff, but stopping builds on
        # D12 left unlocked SW pasture tiles permanently empty.
        if not is_endgame and day <= C4_LIVESTOCK_CUTOFF_DAY + 1:
        ...
        if not is_endgame and next_quadrant is not None:
            if next_quadrant in QUADRANT_HARD_BLOCK:
                ...
                # SW's seed tranche is the treasury buffer. Requiring the
                # generic reserve on top of it delayed the productive window.
                gate_reserve = (0 if next_quadrant == 3 and day <= 11
                                else self.reserve)
                buy_land, sw_reason, sw_info = should_buy_land(
                    ...
                    reserve=gate_reserve, roi=land_roi,
                    ow_factor=ow_factor,
                    forecast=self.fc,
        ...
        if buy_land and next_quadrant == 3:
            effective_reserve = 0
