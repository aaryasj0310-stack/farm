...
class OrderBuilder:
    ...
    def build(self, ctx, intents):
        ...
        # For SW, the explicit feed/hire/seed intents are the safety budget;
        # charging the generic reserve again is what starved the land order.
        order_reserve = (0 if intents.get("buy_land") and "SW" not in farm.unlocked
                         and len(farm.unlocked) == 2 else self.reserve)
        budget = max(0.0, money - order_reserve)
        ...
        available_for_purchases = max(0.0, money - mandatory_hire_budget - order_reserve)
