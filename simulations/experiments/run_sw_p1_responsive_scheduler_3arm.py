"""Balanced-seat three-arm SW scheduler experiment.

Control = exact production baseline.
P1 = exact validated purchase-only correction.
P1.1 = current branch runtime with P1 + workload-responsive scheduler.
MacroPlanner SW activation is deliberately unchanged in P1.1.
"""
...
RESULT_JSON = os.path.join(ROOT, "simulations", "experiments", "results", "sw_p1_responsive_scheduler_3arm_balanced.json")
RESULT_MD = os.path.join(ROOT, "simulations", "experiments", "results", "sw_p1_responsive_scheduler_results.md")
...
def _extract_agent(sha, label):
    sha = _git_sha(sha)
    directory = os.path.join(ROOT, "simulations", "baselines", f"sw_p11_{label}_{sha[:12]}")
...
def _configure_arm(cfg, arm):
    ...
    if hasattr(cfg, "SW_P1_PURCHASE_COMMITTED_HERD_ONLY"):
        cfg.SW_P1_PURCHASE_COMMITTED_HERD_ONLY = arm in ("P1", "P1.1")
    if hasattr(cfg, "set_sw_workload_responsive_scheduler"):
        cfg.set_sw_workload_responsive_scheduler(arm == "P1.1")


def _one(task):
    agent_dir = task["agent_dir"]
    clean = [p for p in sys.path if "simulations/baselines/sw_p11_" not in p.replace("\\", "/")]
...
def _markdown(report):
    ...
    lines = [
        ...
        "| --- | ---: | ---: | ---: |",
        f"| Mean score | USD {arms['Control']['mean_score']:,.2f} | USD {arms['P1']['mean_score']:,.2f} | USD {arms['P1.1']['mean_score']:,.2f} |",
        f"| Confirmed SW acquisitions | {arms['Control']['sw_acquisitions']} | {arms['P1']['sw_acquisitions']} | {arms['P1.1']['sw_acquisitions']} |",
        f"| Starvation observations | {arms['Control']['starvation']} | {arms['P1']['starvation']} | {arms['P1.1']['starvation']} |",
        f"| NW+NE unwatered EOD | {arms['Control']['nw_ne_unwatered_eod']} | {arms['P1']['nw_ne_unwatered_eod']} | {arms['P1.1']['nw_ne_unwatered_eod']} |",
        f"| NW+NE harvest executions | {arms['Control']['nw_ne_harvest_exec']} | {arms['P1']['nw_ne_harvest_exec']} | {arms['P1.1']['nw_ne_harvest_exec']} |",
        f"| SW home-unit turns | {arms['Control']['home_unit_turns']['SW']} | {arms['P1']['home_unit_turns']['SW']} | {arms['P1.1']['home_unit_turns']['SW']} |",
        f"| SW anchor assignments | {arms['Control']['sw_anchor_assignments']} | {arms['P1']['sw_anchor_assignments']} | {arms['P1.1']['sw_anchor_assignments']} |",
        f"| Travel distance | {arms['Control']['travel_distance']} | {arms['P1']['travel_distance']} | {arms['P1.1']['travel_distance']} |", "",
        "## Matched score deltas", "",
    ]
    for key, label in (("p1_vs_control", "P1 - Control"), ("p11_vs_control", "P1.1 - Control"), ("p11_vs_p1", "P1.1 - P1")):
        d = s["deltas"][key]
        lines.append(
...
def run(workers=6):
    import kaggle_environments
    control_sha, p1_sha, p11_sha = _git_sha(BASELINE_SHA), _git_sha(P1_SHA), _git_sha("HEAD")
    dirs = {
    ...
    for case_id, arms in sorted(by_case.items()):
        if all(a in arms and arms[a]["status"] == "SUCCESS" for a in ("Control", "P1", "P1.1")):
            complete.append({
                "case_id": case_id, "pair_id": arms["Control"]["pair_id"],
                "seed": arms["Control"]["seed"], "opponent": arms["Control"]["opponent"],
                "seat": arms["Control"]["seat"], "Control": arms["Control"], "P1": arms["P1"], "P1.1": arms["P1.1"],
    ...
    summary = {
        ...
        "arms": {arm: _aggregate_arm(results, arm) for arm in ("Control", "P1", "P1.1")},
        "deltas": {
    ...
    report = {
        "metadata": {
            "control_sha": control_sha, "p1_sha": p1_sha, "p11_sha": p11_sha,
