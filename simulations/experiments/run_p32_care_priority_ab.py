#!/usr/bin/env python3
"""P3.2 State-Dependent Animal Care Urgency Experiment Runner.

Evaluates Promoted Production Baseline (536f1e7..., QUADRANT_HARD_BLOCK={4}, P23=True)
vs P3.2 State-Dependent Animal Care Urgency (P32_STATE_DEPENDENT_CARE_PRIORITY_ENABLED=True).

100 matched cases (50 pairs x 2 seats = 200 live games).
Fresh seed block: 92,001–92,050.
...
def _extract_control_agent(commit_sha: str) -> str:
    out_dir = os.path.join(tempfile.gettempdir(), f"kagg_p32_control_{commit_sha[:8]}")
...
def _configure_arm(cfg, arm: str):
    """Enforce strict 2Q invariants and set P3.2 flag."""
    ...
    # Set P3.2 switch
    expected_p32 = (arm == "Treatment")
    if hasattr(cfg, "set_p32_state_dependent_care_priority_enabled"):
        cfg.set_p32_state_dependent_care_priority_enabled(expected_p32)
    else:
        setattr(cfg, "P32_STATE_DEPENDENT_CARE_PRIORITY_ENABLED", expected_p32)
    ...
    actual_p32 = getattr(cfg, "P32_STATE_DEPENDENT_CARE_PRIORITY_ENABLED", False)
    assert actual_p32 == expected_p32, f"{arm}: P3.2 expected {expected_p32}, got {actual_p32}"
...
def _verify_manifest(control_dir, worktree_dir):
    for arm, d in (("Control", control_dir), ("Treatment", worktree_dir)):
        ...
        print(f"Verified {arm} config: QUADRANT_HARD_BLOCK={cfg.QUADRANT_HARD_BLOCK}, P23={getattr(cfg, 'P23_MARGINAL_WHEAT_ALLOCATION_ENABLED', False)}, P32={getattr(cfg, 'P32_STATE_DEPENDENT_CARE_PRIORITY_ENABLED', False)}", flush=True)
...
def _one(task):
    ...
    try:
        ...
        m = {
            ...
            "milk_harvest_units": 0,
            "wool_harvest_units": 0,
            "milk_revenue": 0.0,
            "wool_revenue": 0.0,
            "egg_revenue": 0.0,
            "crop_revenue": 0.0,
            ...
            "animal_escapes": 0,
            "realizable_care_missed_count": 0,
        }
        ...
        def tracking(obs, configuration=None):
                    ...
                    for x, t in enumerate(row or []):
                        if isinstance(t, dict):
                            ...
                            if kind == "PLANT":
                                ...
                                if not cared and anim in ("COW", "SHEEP") and day < 29 and fed:
                                    m["realizable_care_missed_count"] += 1
            ...
            if isinstance(action, dict):
                ...
                for u_act in [farmer_act] + hands_acts:
                    op = u_act[0] if isinstance(u_act, (list, tuple)) and len(u_act) > 0 else "PASS"
                    if op == "CARE":
                        m["care_actions_completed"] += 1
                    elif op == "FEED":
                        m["feed_actions_completed"] += 1
                    ...
                    if isinstance(ord_item, (list, tuple)) and len(ord_item) >= 3:
                        ...
                        if op == "SELL":
                            ...
                            if prod == "MILK":
                                ...
                                m["egg_revenue"] += rev
                                m["animal_revenue"] += rev
                            elif prod in ("STRAWBERRY", "MELON", "WHEAT", "CARROT", "TOMATO"):
                                m["crop_revenue"] += rev
        ...
        return {
            ...
            "seat": task["seat"],
            "score": score,
            "care_actions_completed": m["care_actions_completed"],
            "feed_actions_completed": m["feed_actions_completed"],
            "realizable_care_missed_count": m["realizable_care_missed_count"],
...
def run_experiment(num_workers: int = 8, pairs_limit: int = None):
    temp_dir = tempfile.mkdtemp(prefix="kagg_p32_exp_")
    print(f"P3.2 Experiment starting in {temp_dir}", flush=True)
    ...
    ctrl_cares, treat_cares = [], []
    ctrl_milk_rev, treat_milk_rev = [], []
    ctrl_wool_rev, treat_wool_rev = [], []
    ctrl_anim_rev, treat_anim_rev = [], []
    ...
    for key, arm_dict in sorted(paired_runs.items()):
        if "Control" in arm_dict and "Treatment" in arm_dict:
            ...
            ctrl_cares.append(c["care_actions_completed"])
            treat_cares.append(t["care_actions_completed"])

            ctrl_milk_rev.append(c["milk_revenue"])
            treat_milk_rev.append(t["milk_revenue"])
            ctrl_wool_rev.append(c["wool_revenue"])
            treat_wool_rev.append(t["wool_revenue"])
            ctrl_anim_rev.append(c["animal_revenue"])
            treat_anim_rev.append(t["animal_revenue"])
    ...
    print("\n" + "=" * 78)
    print(f"  P3.2 EXPERIMENT RESULTS: {n} MATCHED CASES ({total_games} GAMES)")
    print("=" * 78)
    print(f"Control Baseline (536f1e7) Mean : ${mean_c:,.2f}")
    print(f"Treatment (P3.2 Care Urgency) Mean: ${mean_t:,.2f}")
    ...
    mean_c_care = statistics.mean(ctrl_cares)
    mean_t_care = statistics.mean(treat_cares)
    print(f"CARE Actions / Game: Control={mean_c_care:.1f} -> Treatment={mean_t_care:.1f} (Delta: +{mean_t_care - mean_c_care:.1f})")

    mean_c_anim_rev = statistics.mean(ctrl_anim_rev)
    mean_t_anim_rev = statistics.mean(treat_anim_rev)
    print(f"Animal Revenue     : Control=${mean_c_anim_rev:,.2f} -> Treatment=${mean_t_anim_rev:,.2f} (Delta: +${mean_t_anim_rev - mean_c_anim_rev:,.2f})")

    mean_c_crop_rev = statistics.mean(ctrl_crop_rev)
    mean_t_crop_rev = statistics.mean(treat_crop_rev)
    print(f"Crop Revenue       : Control=${mean_c_crop_rev:,.2f} -> Treatment=${mean_t_crop_rev:,.2f} (Delta: +${mean_t_crop_rev - mean_c_crop_rev:,.2f})")
    ...
    # Output JSON payload
    out_json = os.path.join(ROOT, "simulations", "experiments", "results", "p32_care_priority_ab.json")
    ...
    payload = {
        ...
        "summary": {
            ...
            "care_actions_delta": mean_t_care - mean_c_care,
            "animal_revenue_delta": mean_t_anim_rev - mean_c_anim_rev,
            "crop_revenue_delta": mean_t_crop_rev - mean_c_crop_rev,
        },
        "opponent_breakdown": {
            opp: {
        ...
        "matched_cases": [
            {
                ...
                "control_cares": arm_dict["Control"]["care_actions_completed"],
                "treatment_cares": arm_dict["Treatment"]["care_actions_completed"],
                "control_anim_rev": arm_dict["Control"]["animal_revenue"],
                "treatment_anim_rev": arm_dict["Treatment"]["animal_revenue"],
                "control_crop_rev": arm_dict["Control"]["crop_revenue"],
                "treatment_crop_rev": arm_dict["Treatment"]["crop_revenue"],
            }
            for key, arm_dict in sorted(paired_runs.items())
            if "Control" in arm_dict and "Treatment" in arm_dict
        ]
